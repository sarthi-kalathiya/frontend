...
export class UsersComponent implements OnInit, OnDestroy {
  ...
  toggleActionMenu(userId: string, event?: MouseEvent): void {
    ...
    if (event) {
      ...
      const shouldFlip =
        buttonRect.bottom + estimatedMenuHeight > viewportHeight;
        ...
        if (confirmed) {
          this.userService.updateUserStatus(userId, newStatus).subscribe({
            ...
            error: (error) => {
              ...
              this.toastService.showError(
                error.message || `Failed to ${actionName} user`
              );
            },
          });
        }
