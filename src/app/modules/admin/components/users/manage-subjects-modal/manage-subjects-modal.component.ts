...
export class ManageSubjectsModalComponent implements OnInit {
  ...
  loadData(): void {
    ...
    this.userService.getUserById(this.userId).subscribe({
      next: (userResponse) => {
        ...
        this.originalAssignedSubjectIds = this.assignedSubjects.map(
          (s) => s.id
        );

        // Then load all available subjects
        this.subjectService
          .getAllSubjects({ includeInactive: false })
          .subscribe({
  ...
  filterAvailableSubjects(): void {
    ...
    this.filteredAvailableSubjects = this.availableSubjects.filter(
      (subject) =>
  ...
  removeSubject(subjectId: string): void {
    ...
    const removedSubject = this.assignedSubjects.find(
      (s) => s.id === subjectId
    );
    if (removedSubject) {
      ...
      const alreadyExists = this.availableSubjects.some(
        (s) => s.id === subjectId
      );
      if (!alreadyExists) {
  ...
  saveChanges(): void {
    ...
    this.subjectService
      .assignSubjectsToUser(this.userId, subjectIds)
      .subscribe({
        next: (response) => {
          this.toastService.showSuccess(
            `Successfully updated subject assignments for ${this.userName}`
          );
          this.isSaving = false;
